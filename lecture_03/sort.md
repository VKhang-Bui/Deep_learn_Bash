# Lệnh `sort` - Cỗ Máy Sắp Xếp Dữ Liệu

Đúng như thỏa thuận "Masterclass" của chúng ta, lệnh `sort` sẽ được mổ xẻ thật sâu và tách biệt khỏi `uniq` để bạn có thể lĩnh hội trọn vẹn bản chất. Lệnh `sort` nhìn bề ngoài rất thân thiện, nhưng lại mang trong mình những cơ chế và "cạm bẫy" chết người đối với dân làm Big Data.

---

### 1. Bản chất cốt lõi (Under the hood)
Lệnh `sort` làm nhiệm vụ đọc văn bản và sắp xếp lại toàn bộ các dòng theo thứ tự từ điển (A-Z).

**Bí mật bộ nhớ (Memory Constraint):** 
Khác với `grep` hay `cut` là các công cụ Stream Processing (đọc và ném đi từng dòng một, cực tiết kiệm RAM), lệnh `sort` **BẮT BUỘC phải ngậm TOÀN BỘ dữ liệu vào RAM** rồi mới bắt đầu xào nấu sắp xếp. Lấy ví dụ: Làm sao bạn biết ai cao nhất lớp nếu bạn chưa đo hết tất cả học sinh?

*Câu hỏi đặt ra:* Nếu bạn sắp xếp một file FASTQ 500GB trên máy tính chỉ có 8GB RAM, máy có sập không? 
*Trả lời:* Rất may là Không! Lệnh `sort` sở hữu một thuật toán vĩ đại gọi là **External Merge Sort**. Khi nó thấy RAM sắp đầy, nó tự động chặt dữ liệu đang giữ ra thành các file nhỏ, nhét tạm vào ổ cứng (vào thư mục `/tmp`), sắp xếp từng mẩu đó rồi gộp lại. 
*Hệ quả:* `sort` cực kỳ an toàn cho RAM nhưng lại **rất tốn thời gian chạy** và "ăn" tốc độ ổ cứng cực mạnh.

---

### 2. Các tùy chọn (Options) "Sinh tử"
Mặc định `sort` sắp xếp mọi thứ theo kiểu "Chữ cái" (Lexicographical). 

**A. `-n` (Numeric - Sắp xếp theo số học)**
*Cạm bẫy:* Mặc định, máy tính coi chuỗi `"10"` đứng trước chuỗi `"2"` (vì chữ số 1 đứng trước chữ số 2 trong bảng chữ cái). 
Nếu bạn muốn sắp xếp các con số tọa độ Gen thực sự, bạn BẮT BUỘC phải dùng `-n` để đánh thức tư duy toán học của `sort`.
...
**C. `-V` (Version - Phép thuật của Tin Sinh Học)**
Đây là vị cứu tinh của các nhà sinh học! 
...
**CẠM BẪY LỚN NHẤT CỦA CÚ PHÁP `-k`:**
...
*Bổ trợ siêu cấp:* Bash cho phép bạn nhét thẳng Option `-n` hay `-r` vào đuôi của khai báo Cột!
Ví dụ: `-k 2,2n` (Sắp xếp theo Cột 2, và ép Cột 2 phải đối xử như Số học).

---

### 4. Ứng dụng Thực chiến và Đường ống (Pipeline)
